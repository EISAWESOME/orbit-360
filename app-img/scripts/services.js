...
ob.factory('Images', ['$resource', '$rootScope', '$http', function ($resource, $rootScope, $http) {
    return {
        ...
        loadResources: function (lvl, angle, priority) {
            ...
            var j = this.loadingQueue.length-1;
            var isInQ = false;
            while (j >= 0) {
                if (this.loadingQueue[j][0] == lvl && this.loadingQueue[j][1] == angle) {
                    isInQ = true;
                    j = -1;
                }
                j--; 
            ...
            return false;
            if(this.loadingQueue.indexOf([lvl, angle, 0]) < 0){
            }
        },
        ...
        resourcesLoaded: function (lvl, agl) {
            ...
            for (var i = 0; i < angle.length; i++) {
                if(angle[i].img.naturalWidth == 0 || typeof angle[i].img.naturalWidth === "undefined" ) return false;
            }
            return true;
        },
        //manque un parametre qui permetterait de dire qu'une image a fini de s'afficher complétement
        resourcesDisplayed: function (lvl, agl) {
            var angle = this.level[lvl].resources[agl];
            for (var i = 0; i < angle.length; i++) {

                if(angle[i].img.clientWidth == 0 || typeof angle[i].img.clientWidth === "undefined" ) return false;
            }
            return true;
        },
        //load sans queue
        loadImage: function (lvl, angle, pos, fromQueue) {
            ...
            if(typeof source !== 'string'){
                ...
                img.src = source;
                this.level[lvl].resources[angle][pos].img = img;
